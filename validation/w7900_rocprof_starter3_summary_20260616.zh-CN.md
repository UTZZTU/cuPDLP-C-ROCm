# W7900 rocprof starter3 摘要

结果目录：`/app/cupdlp_w7900/results/w7900_rocprof/w7900_rocprof_starter3_20260616_172230_20260616_172230`

本文是 W7900 `rocprof` starter3 运行结果的 compact summary。原始 profiler trace 文件体积很大，不进入 Git。
...
- `set-cover-model` 和 `square41` 在 profiling 运行中达到 `OPTIMAL`。
- `s100` 的 solver 在约 900 秒达到时间上限，relative duality gap 约为 `4.94e-04`；其 profiler stats 文件已经生成，但外层 profiler wrapper 在 trace finalization 阶段以 timeout code `124` 结束。
- 原始 HIP API trace 和 kernel trace 文件体积很大，应保存在 Git 外部；Git 中只提交 compact CSV 和 Markdown summary。
